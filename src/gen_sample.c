...
static struct event_desc  events[] = {
	{.type=PERF_TYPE_HARDWARE, .config=PERF_COUNT_HW_CPU_CYCLES,   .name="cycles"},
	{.type=PERF_TYPE_HARDWARE, .config=PERF_COUNT_HW_INSTRUCTIONS, .name="instructions"}
};
...
setup_perf(int index, unsigned int type, unsigned int config)
{
	...
	event_attr[index].sample_period = 100;
...
int main(int argc, char *argv[])
{
	...
	setup_perf(0, events[0].type, events[0].config);
	setup_perf(1, events[1].type, events[1].config);
